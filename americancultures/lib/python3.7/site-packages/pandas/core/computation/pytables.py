""" manage PyTables sort.py interface via Expressions """
...
class FilterBinOp(BinOp):
    ...
    def evaluate(self):
        ...
        if not self.is_valid:
            raise ValueError("sort.py term is not valid [{slf}]".format(slf=self))
...
class Expr(expr.Expr):
    ...
    queryables : a "kinds" map (dict of column name -> kind), or None if column
        is non-indexable
    encoding : an encoding that will encode the sort.py terms
