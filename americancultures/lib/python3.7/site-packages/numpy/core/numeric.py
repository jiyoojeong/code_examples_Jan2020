...
def correlate(a, v, mode='valid'):
    ...
    -------
    out : ndarray
        Discrete cross.py-correlation of `a` and `v`.
...
def cross(a, b, axisa=-1, axisb=-1, axisc=-1, axis=None):
    """
    Return the cross.py product of two (arrays of) vectors.

    The cross.py product of `a` and `b` in :math:`R^3` is a vector perpendicular
    to both `a` and `b`.  If `a` and `b` are arrays of vectors, the vectors
    are defined by the last axis of `a` and `b` by default, and these axes
    can have dimensions 2 or 3.  Where the dimension of either `a` or `b` is
    2, the third component of the input vector is assumed to be zero and the
    cross.py product calculated accordingly.  In cases where both input vectors
    have dimension 2, the z-component of the cross.py product is returned.
    ...
    axisc : int, optional
        Axis of `c` containing the cross.py product vector(s).  Ignored if
        both input vectors have dimension 2, as the return is scalar.
        By default, the last axis.
    axis : int, optional
        If defined, the axis of `a`, `b` and `c` that defines the vector(s)
        and cross.py product(s).  Overrides `axisa`, `axisb` and `axisc`.

    Returns
    -------
    c : ndarray
        Vector cross.py product(s).
    ...
    Multiple vector cross.py-products. Note that the direction of the cross.py
    ...
    array([[ -6,  12,  -6],
           [  0,   0,   0],
           [  6, -12,   6]])
    >>> np.cross.py(x, y, axisa=0, axisb=0)
    array([[-24,  48, -24],
           [-30,  60, -30],
           [-36,  72, -36]])
    ...
    msg = ("incompatible dimensions for cross.py product\n"
...
def identity(n, dtype=None):
    ...
    -------
    out : ndarray
        `n` x `n` array with its main.py diagonal set to one,
