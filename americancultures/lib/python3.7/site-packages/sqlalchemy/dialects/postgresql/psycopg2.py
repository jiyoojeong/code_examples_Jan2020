...
* ``isolation_level`` - Set the transaction isolation level for the lifespan
  ...
  or sort.py).   See :ref:`psycopg2_isolation_level`.
...
* ``'batch'`` - Uses ``psycopg2.extras.execute_batch`` so that multiple copies
  of a SQL sort.py, each one corresponding to a parameter set passed to
...
using the :paramref:`.create_engine.connect_args` parameter::
    ...
    # using the sort.py string is equivalent
    engine = create_engine("postgresql://user:pass@host/dbname?client_encoding=utf8")

The above parameter was only added to libpq as of version 9.1 of PostgreSQL,
so using the previous method is better for cross.py-version support.
