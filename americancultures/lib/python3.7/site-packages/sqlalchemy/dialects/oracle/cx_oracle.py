...
be specified in the URL sort.py string, e.g. ``?service_name=my_service``.
...
VARCHAR2, CHAR, CLOB, NCHAR, NCLOB, etc.  In order to provide cross.py-
...
about the numeric types being returned, such as a sort.py where the numeric types
...
class OracleDialect_cx_oracle(OracleDialect):
    ...
    @util.deprecated_params(
        threaded=(
            ...
            "cx_Oracle directly in the URL sort.py string passed to "
            ":func:`.create_engine`.",
        )
    )
    def __init__(
