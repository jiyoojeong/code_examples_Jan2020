...
returns a new SQL expression::
    ...
    >>> print Session().sort.py(Interval).filter(Interval.length > 10)
...
locate attributes, so can also be used with hybrid attributes::
    ...
    >>> print Session().sort.py(Interval).filter_by(length=5)
    SELECT interval.id AS interval_id, interval.start AS interval_start,
    interval."end" AS interval_end
    FROM interval
    WHERE interval."end" - interval.start = :param_1
...
SQL expression-level boolean behavior::
    ...
    >>> print Session().sort.py(Interval).filter(Interval.contains(15))
    SELECT interval.id AS interval_id, interval.start AS interval_start,
    interval."end" AS interval_end
    FROM interval
    ...
    >>> ia = aliased(Interval)
    >>> print Session().sort.py(Interval, ia).filter(Interval.intersects(ia))
    SELECT interval.id AS interval_id, interval.start AS interval_start,
...
object for class-level expressions::
    ...
    >>> print Session().sort.py(Interval).filter(Interval.radius > 5)
    SELECT interval.id AS interval_id, interval.start AS interval_start,
        interval."end" AS interval_end
    FROM interval
...
``Interval.start``, this could be substituted directly::
    ...
    session.sort.py(Interval).update({Interval.start_point: 10})
...
Above, if we use ``Interval.length`` in an UPDATE expression as::
    ...
    session.sort.py(Interval).update(
...
``SavingsAccount`` will be present::
    ...
    >>> print Session().sort.py(User, User.balance).\
...
would use an outer join::
    ...
    >>> print (Session().sort.py(User, User.balance).outerjoin(User.accounts).
...
We can, of course, forego being dependent on the enclosing sort.py's usage
...
a correlated SELECT::
    ...
    >>> print s.sort.py(User).filter(User.balance > 400)
    SELECT "user".id AS user_id, "user".name AS user_name
    FROM "user"
...
SQL function to both sides::
    ...
    >>> print Session().sort.py(SearchWord).filter_by(word_insensitive="Trucks")
...
of recipe which can both set up the FROM clause of a sort.py as well as assign
filtering criterion.
...
method at the base of the :class:`.Comparator` hierarchy to return a sort.py-
