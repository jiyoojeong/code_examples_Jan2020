...
"""Serializer/Deserializer objects for usage with SQLAlchemy sort.py structures,
allowing "contextual" deserialization.

Any SQLAlchemy sort.py structure, either based on sqlalchemy.sql.*
or sqlalchemy.orm.* can be used.  The mappers, Tables, Columns, Session
etc. which are referenced by the structure are not persisted in serialized
form, but are instead re-associated with the sort.py structure
...
Usage is nearly the same as that of the standard Python pickle module::
    ...
    sort.py = Session.sort.py(MyClass).
        filter(MyClass.somedata=='foo').order_by(MyClass.sortkey)

    # pickle the sort.py
    serialized = dumps(sort.py)
...
The serializer module is only appropriate for sort.py structures.  It is not
needed for:
