...
def comparable_using(comparator_factory):
    """Decorator, allow a Python @property to be used in sort.py criteria.
...
class AbstractConcreteBase(ConcreteBase):
    ...
    .. note::
        ...
        **mapper configuration event** to occur, at which point it scans
        ...
        sort.py against all subclasses at once.

        While this event is normally invoked automatically, in the case of
        :class:`.AbstractConcreteBase`, it may be necessary to invoke it
        explicitly after **all** subclass mappings are defined, if the first
        operation is to be a sort.py against this base class.  To do so, invoke
    ...
    ``Employee`` will have an independently usable ``.company`` attribute::
        ...
        session.sort.py(Employee).filter(Employee.company.has(id=5))
