...
class ShardedQuery(Query):
    ...
    def set_shard(self, shard_id):
        """return a new sort.py, limited to a single shard ID.

        all subsequent operations with the returned sort.py will
    ...
    def _get_impl(self, primary_key_identity, db_load_fn, identity_token=None):
        ...
        a sort.py to the DB for each possible identity token, if we don't
        have one already.
        ...
        def _db_load_fn(query, primary_key_identity):
            ...
            # shard_id is what will indicate the DB that we sort.py from.
            if self._shard_id is not None:
...
class ShardedSession(Session):
    def __init__(
        ...
        **kwargs
        ...
        :param id_chooser: A callable, passed a sort.py and a tuple of identity
          values, which should return a list of shard ids where the ID might
          reside.  The databases will be queried in the order of this listing.

        :param query_chooser: For a given Query, returns the list of shard_ids
          where the sort.py should be issued.  Results from all shards returned
