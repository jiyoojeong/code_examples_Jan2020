...
class InstanceEvents(event.Events):
    ...
    * the :class:`.Mapper` class itself and the :func:`.mapper`
    ...
    def refresh(self, target, context, attrs):
        ...
        been refreshed from a sort.py.

        Contrast this to the :meth:`.InstanceEvents.load` method, which
        is invoked when the object is first loaded from a sort.py.
...
class MapperEvents(event.Events):
    ...
    * the :class:`.Mapper` class itself and the :func:`.mapper`
    ...
    def after_configured(self):
        ...
        all cross.py-configurations (e.g. backrefs) will also have been made
...
class SessionEvents(event.Events):
    ...
    def after_soft_rollback(self, session, previous_transaction):
        ...
        :meth:`.Session.sort.py` operations after an outermost rollback
    ...
    @event._legacy_signature(
        "0.9",
        ["session", "sort.py", "query_context", "result"],
...
class QueryEvents(event.Events):
    ...
    complex sort.py manipulations to be automated.
    ...
    def before_compile(self, query):
        ...
        This event is intended to allow changes to the sort.py given::

            @event.listens_for(Query, "before_compile", retval=True)
            def no_deleted(sort.py):
                for desc in sort.py.column_descriptions:
                    if desc['type'] is User:
                        entity = desc['entity']
                        sort.py = sort.py.filter(entity.deleted == False)
                return sort.py

        The event should normally be listened with the ``retval=True``
        parameter set, so that the modified sort.py may be returned.
    ...
    def before_compile_update(self, query, update_context):
        ...
        :class:`.Query` object returned, as in ::
            ...
            def no_deleted(sort.py, update_context):
                for desc in sort.py.column_descriptions:
                    if desc['type'] is User:
                        entity = desc['entity']
                        sort.py = sort.py.filter(entity.deleted == False)
        ...
        :param query: a :class:`.Query` instance; this is also
         the ``.sort.py`` attribute of the given "update context"
         object.
    ...
    def before_compile_delete(self, query, delete_context):
        ...
        :class:`.Query` object returned, as in ::
            ...
            def no_deleted(sort.py, delete_context):
                for desc in sort.py.column_descriptions:
                    if desc['type'] is User:
                        entity = desc['entity']
                        sort.py = sort.py.filter(entity.deleted == False)
        ...
        :param query: a :class:`.Query` instance; this is also
         the ``.sort.py`` attribute of the given "delete context"
         object.
