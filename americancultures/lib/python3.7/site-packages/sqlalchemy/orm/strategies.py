...
class LazyLoader(AbstractRelationshipLoader, util.MemoizedSlots):
    ...
    def __init__(self, parent, strategy_key):
        ...
        if self.use_get:
            ...
            self.logger.info(
                "%s will use sort.py.get() to " "optimize instance loads", self
            )
    ...
    def _emit_lazyload(
        ...
        #    can be different due to the options passed to the sort.py, which
        ...
        #    "throwaway" object, like a per-sort.py AliasedClass, meaning
        ...
        # set parameters in the sort.py such that we don't overwrite
...
class SubqueryLoader(AbstractRelationshipLoader):
    ...
    def setup_query(
        ...
        **kwargs
        ...
        # if not via sort.py option, check for
        # a cycle
        ...
        # add new sort.py to attributes to be picked up
    ...
    def _generate_from_original_query(
        ...
        # reformat the original sort.py
        # to look only for significant columns
        q = orig_query._clone().correlate(None)

        # set the sort.py's "FROM" list explicitly to what the
        ...
        # other columns from the sort.py that might suggest the right entity
        ...
        # the original sort.py now becomes a subquery
    ...
    def _setup_options(self, q, subq_path, orig_query, effective_entity):
        # propagate loader options etc. to the new sort.py.
    ...
    def _setup_outermost_orderby(self, q):
        if self.parent_property.order_by:
            ...
            # the "eagerjoin" from the sort.py.
    ...
    class _SubqCollections(object):
        ...
        provide a load interface that executes the sort.py at the
...
class JoinedLoader(AbstractRelationshipLoader):
    ...
    def _setup_query_on_user_defined_adapter(
        ...
        # if we are setting up the sort.py for SQL render.
    ...
    def _gen_pooled_aliased_class(self, context):
        ...
        # we need one unique AliasedClass per sort.py per appearance of our
        # entity in the sort.py.
    ...
    def _create_eager_join(
        ...
        if (
            ...
            if len(indexes) > 1:
                ...
                # now.   For sort.py.join() I can.
                raise sa_exc.InvalidRequestError(
                    "Can't identify which sort.py entity in which to joined "
            ...
            if indexes:
                ...
                # join to an existing FROM clause on the sort.py.
