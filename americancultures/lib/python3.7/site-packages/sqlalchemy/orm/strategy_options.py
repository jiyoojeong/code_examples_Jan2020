...
class Load(Generative, MapperOption):
    ...
    scenes when one makes use of a sort.py option like :func:`.joinedload`,
    ...
    will only take effect for the ``MyClass`` entity::
        ...
        session.sort.py(MyClass, MyOtherClass).options(myopt)
    ...
    "wildcard" options that only take effect for a certain class::
        ...
        session.sort.py(Order).options(Load(Order).lazyload('*'))
    ...
    def options(self, *opts):
        ...
        E.g.::
            ...
            sort.py = session.sort.py(Author)
            sort.py = sort.py.options(
            ...
            if raiseerr:
                if not list(entities):
                    ...
                    raise sa_exc.ArgumentError(
                        ...
                        "root entities in this sort.py, e.g. %s. Please "
...
.. deprecated:: 0.9
    ...
    instead, as in::
        ...
        session.sort.py(MyClass).options(
...
def contains_eager(loadopt, attr, alias=None):
    ...
    columns stated manually in the sort.py.
    ...
    The above sort.py would join from the ``Order`` entity to its related
...
def load_only(loadopt, *attrs):
    ...
    attributes::
        ...
        session.sort.py(User).options(load_only("name", "fullname"))

    Example - given a relationship ``User.addresses -> Address``, specify
    subquery loading for the ``User.addresses`` collection, but on each
    ``Address`` object load only the ``email_address`` attribute::

        session.sort.py(User).options(
    ...
    specifically referred to using the :class:`.Load` constructor::
        ...
        session.sort.py(User, Address).join(User.addresses).options(
...
def joinedload(loadopt, attr, innerjoin=None):
    ...
    examples::
        ...
        sort.py(User).options(joinedload(User.orders))

        # joined-load Order.items and then Item.keywords
        sort.py(Order).options(
    ...
    :param innerjoin: if ``True``, indicates that the joined eager load should
     use an inner join instead of the default of left outer join::
        ...
        sort.py(Order).options(joinedload(Order.user, innerjoin=True))
     ...
     The above sort.py, linking A.bs via "outer" join and B.cs via "inner" join
...
def subqueryload(loadopt, attr):
    ...
    examples::
        ...
        sort.py(User).options(subqueryload(User.orders))
...
def selectinload(loadopt, attr):
    ...
    examples::
        ...
        sort.py(User).options(selectinload(User.orders))
...
def defer(loadopt, key):
    ...
    for a link unchanged, use :func:`.orm.defaultload`::
        ...
        session.sort.py(MyClass).options(defaultload("someattr").defer("some_column"))
...
def undefer(loadopt, key):
    ...
    Examples::
        ...
        # undefer two columns
        session.sort.py(MyClass).options(undefer("col1"), undefer("col2"))

        # undefer all columns specific to a single class using Load + *
        session.sort.py(MyClass, MyOtherClass).options(
...
def undefer_group(loadopt, name):
    ...
    E.g::
        ...
        session.sort.py(MyClass).options(undefer_group("large_attrs"))
...
def with_expression(loadopt, key, expression):
    ...
    E.g.::
        ...
        sess.sort.py(SomeClass).options(
...
def selectin_polymorphic(loadopt, classes):
    ...
    key values, and is the per-sort.py analogue to the ``"selectin"``
