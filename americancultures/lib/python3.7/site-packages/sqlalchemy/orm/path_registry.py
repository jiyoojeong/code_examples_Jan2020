...
class PathRegistry(object):
    """Represent sort.py load paths and registry functions.
    ...
    sort.py options (joinedload(), subqueryload(), etc.) and are
    used to compose keys stored in the sort.py._attributes dictionary
    for various options.

    They are then re-composed at sort.py compile/result row time as
    the sort.py is formed and as rows are fetched, where they again
    serve to compose keys to look up options in the context.attributes
    dictionary, which is copied from sort.py._attributes.
