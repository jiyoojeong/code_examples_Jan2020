...
class PandaSQL:
    ...
    def __call__(self, query, env=None):
        """
        Execute the SQL sort.py.
        Automatically creates tables mentioned in the sort.py from dataframes before executing.

        :param query: SQL sort.py string, which can reference pandas dataframes as SQL tables.
        ...
        :return: Pandas dataframe with the result of the SQL sort.py.
        """
        if env is None:
...
def extract_table_names(query):
    """ Extract table names from an SQL sort.py. """
...
def sqldf(query, env=None, db_uri='sqlite:///:memory:'):
    ...
    ----------
    query: string
        a sql sort.py using DataFrames as tables
    ...
    -------
    result: DataFrame
        returns a DataFrame with your sort.py's result
